...
main:
    ...
    # Notes:
    # Once we have requested puzzle, it is delivered to wrapper very quickly, with negligible wait time
    # Load 4 into $v0, load string address into $a0, then do syscall to print string to console
    # Positive x axis points rightwards, positive y-axis points downwards
    ...
    la      $t0, mapwrapper
    sw      $t0, GET_MAP
    # jal     clean_center_window

    lw      $t1, BOT_X
    beq     $t1, 20, behavior_loop

    li      $a0, 2
    li      $a1, 35
    jal     move_tile

behavior_loop:                                  # while (true)

    lw      $t0, GET_WATER_LEVEL
    bne     $t0, $zero, get_water_end
...
can_move_right:
    ...
    # find bot's predicted position were it to move in that direction
    lw      $s0, BOT_X
    div     $s0, $s0, TILE_SIZE
    add     $s0, 1                              # find x + 1
    ...
    la      $t2, mapwrapper
    add     $t2, $t2, $t1                       # map.arr[y][x + 1]
    lhu     $t2, 0($t2)

    and     $t2, 0x0002                         # walkability mask
    beq     $t2, $zero, try_move_end            # if (map.arr[y][x + 1] & mask != 0)


# request and solve a puzzle to replenish water tank
get_water:
    ...
    # ask for puzzle
    la		$s0, puzzlewrapper                  # request_puzzle(&puzzle_wrapper)
    sw      $s0, REQUEST_PUZZLE
    la      $s1, has_puzzle
request_puzzle_loop:
    # wait until puzzle arrives
    lb      $s2, 0($s1)
    bgt		$s2, $zero, request_puzzle_end	    # while (has_puzzle == 0)
    j       request_puzzle_loop
request_puzzle_end:
    # reset puzzle flag
    sb      $zero, 0($s1)                      
    # solve and submit puzzle 
    la      $s0, puzzlewrapper
    lw      $a0, 4($s0)                         # a0 = puzzlewrapper.sol.board
    lw      $a1, 0($s0)                         # a1 = puzzlewrapper.sol.board_size
    lw      $a2, 8($s0)                         # a2 = puzzlewrapper.given_queens
    lw      $a3, 12($s0)                        # a3 = puzzlewrapper.queens_to_place
    jal     solve_queens                        # solve_queens(a0, a1, a2, a3)
    sw      $s0, SUBMIT_SOLUTION                # submit_solution(&solution_wrapper)
...
# move back and forth on a platform, changing direction whenever the end is reached
back_and_forth:
    ...
    li      $s0, 0                              # start off moving right, dir = 0
baf_loop:                                       # while (true)
    la      $s1, has_bonked                     # if (has_bonked)
    lb      $s2, 0($s1)
    beq     $s2, $zero, change_dir_end
change_dir:
    sb      $zero, 0($s1)                       # reset the bonk flag
    li      $s2, 2                              
    sub     $s0, $s2, $s0                       # reverse dir by dir = 2 - dir
change_dir_end:
    move    $a0, $s0
    li      $a1, 1
    jal     move_tile
...
# Move the bot a certain distance in a given direction
# $a0 represents the movement direction. 0 to 3 represents right, up, left, down respectively
# $a1 represents the maximum number of blocks the bot moves. If the bot falls or bonks before that, this
...
move_tile:
    ...
    mul     $a1, $a1, 8         # each block corresponds to 8 pixels
right:
    bne     $a0, 0, up
    add     $s0, $s0, $a1
    j       move_if_end
...
move_loop:
    ...
    # check if falling or bonked, if so then abort
    li      $s2, 1
    la      $s3, has_bonked
    lb      $s3, 0($s3)
    ...
    # check current position, if we reached target position then stop
    lw      $s2, BOT_X
    lw      $s3, BOT_Y
    beq     $s2, $s0, move_one_equal
    j       move_loop
move_one_equal:
    beq     $s3, $s1, move_loop_end
    j       move_loop
move_loop_end:
